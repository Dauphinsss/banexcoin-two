...
export class AnomalyExplainerAgent {
  ...
  private getClient(): Promise<CerebrasClient | null> {
    ...
    if (!apiKey) {
      this.logger.warn('CEREBRAS_API_KEY no está configurada')
  ...
  private async prepareExplanation(uploadId: string): Promise<PreparedExplanation> {
    ...
    if (!client) {
      this.logger.warn('Cliente de Cerebras no está disponible, usando explicación determinística')
      return {
        kind: 'static',
        available: false,
        cached: false,
  ...
  const amountContext = deltas.length > 0
    ? ` En diferencias de monto, el delta promedio es Bs ${(deltas.reduce((sum, value) => sum + value, 0) / deltas.length).toFixed(2)
